...
// TC: O(log(min(a,b)))
// SC: O(log(min(a,b)))
//This logarithmic time complexity arises because each recursive step reduces the problem size roughly by half (specifically, by the size of the smaller number)
